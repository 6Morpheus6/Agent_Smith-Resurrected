#!/usr/bin/env node
/**
 * _repro-mobile-websearch — reproduce "mobile web UI shows sources but no results"
 * against the LIVE LM Studio backend, exactly as the renderer's chat loop sends it:
 *   turn 1: system(agent appendix) + user("Search the web for …") → expect web_search tool call
 *   turn 2: + assistant(tool_calls) + tool(compact result text + nudge) → expect final answer
 *
 * Usage: node scripts/_repro-mobile-websearch.js [think-on|think-off]
 */
'use strict';

const BASE = process.env.LMS_BASE || 'http://127.0.0.1:1234/v1/chat/completions';
const MODEL = process.argv[2] === 'think-on' ? 'qwen3.8-27b-obliterated' : 'qwen3.8-27b-obliterated';
const THINK_ON = process.argv[2] !== 'think-off';

// Same tool schema the renderer sends (agentTools.js).
const TOOLS = [{
    type: 'function',
    function: {
        name: 'web_search',
...
async function streamTurn(messages, label) {
    ...
    console.log(`\n── ${label} (${secs}s, finish=${finishReason}) ──`);
    console.log(`   content:   ${content.length} chars${content ? ` → "${content.slice(0, 220)}"` : '  (EMPTY)'}`);
...
(async () => {
    ...
    const t1 = await streamTurn([
        { role: 'system', content: SYSTEM },
        { role: 'user', content: USER_MSG }
    ], 'TURN 1 — user asks for a web search');

    if (!t1.toolCalls?.length) {
        console.log('\nModel did not call web_search on turn 1 — cannot continue repro.');
        process.exit(2);
    }
