...
const CTX_STEP = 1024;
// AGENT SMITH RESURRECTED: base context is 16K (step-aligned to 16384). No Code Mode in this
// build, so the agent chat path runs from 16K upward — and auto-tune must NEVER compute a
// window below src/shared/ctxGate.js's minimum (16000), or it would reload models into a
// state the send-time gate blocks. CTX_MIN is that floor for every computed profile.
const CTX_MIN = 16384;
