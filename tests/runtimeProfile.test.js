...
test('snapCtx rounds to 1024 step within bounds', () => {
    // Resurrected: CTX_MIN is the 16K base ctx floor — anything below snaps up to it.
    assert.equal(snapCtx(20000), 20480);
    assert.equal(snapCtx(1000), 16384);
});
...
test('general 7B + low VRAM sits at the resurrected base ctx (no Code Mode → 16K floor)', () => {
    const p = buildRuntimeProfile({
        modelId: 'llama-3.2-7b-instruct',
        telemetry: { memory: { total: 6144 } }
    });
    assert.equal(p.numCtx, 16384);
